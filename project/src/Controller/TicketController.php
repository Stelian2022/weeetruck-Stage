...
class TicketController extends AbstractController
{
    ...
    public function new(Request $request, TicketRepository $ticketRepository, Security $security): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($imageFile) {
                ...
                //Deplacer l'image dans le dossier
                $imageFile->move(
                    $this->getParameter('image_directory'), // Definie dans config/services.yaml file
                    $newFilename
                );
    ...
    public function edit(Request $request, Ticket $ticket, TicketRepository $ticketRepository, Mailer $mailer): Response
    {
        $form = $this->createForm(TicketType::class, $ticket);
        $form->handleRequest($request);
